...
const dishes = [
    ...
    {
        "name": "Dame el balón normal",
        "ingredients": [
            ingredients[2]
        ]
    },
    {
        // "name": "Bolsa con guantes y Tenis",
        "name": "Quiero el balón azul",
        "ingredients": [
            ingredients[1]
        ]
    },
    {
        "name": "Dame un balón negro",
        "ingredients": [
            ingredients[0],
        ]
    },
    {
        "name": "Quiero todos los balones!",
        "ingredients": [
            ingredients[0],
            ingredients[1],
            ingredients[2],
...
let RandomDish1 = getRandomInt(2,5);
let RandomDish2 = getRandomInt(2,5);
let RandomDish3 = getRandomInt(2,5);
let RandomDish4 = getRandomInt(2,5);
let RandomDish5 = getRandomInt(2,5);
let RandomDish6 = getRandomInt(2,5);
let RandomDish7 = getRandomInt(2,5);
...
const items = [
    {
        ...
        "duration": 10000,
        "mesh": "Models/PowerUps/Rayo2_PowerUp.glb",
        "boundingBox": "",
        "spawned": true
    },
    {
        ...
        "duration": 30000,
        "mesh": "Models/PowerUps/balon_dor.glb",
        "boundingBox": "",
        "spawned": true
    },
    {
        ...
        "duration": 1,
        "mesh": "Models/PowerUps/reloj_tiempo.glb", 
        "boundingBox": "",
        "spawned": true
    },
]
...
//MODELO PARA AGARRAR LAS BOLSAS
let platoMesh;
let platoBB
new GLTFLoader().load('Models/Items/bolsa.glb', function(gltf){
    const model = gltf.scene;
    model.traverse(object=>{
        if(object.isMesh) {
            object.castShadow=true;
...
//Balon negro
let arrozMesh
new GLTFLoader().load('Models/Items/balon_negro.glb', function(gltf){
    const model = gltf.scene;
    model.traverse(object=>{
        if(object.isMesh) {
            object.castShadow=true;
...
//Balon azul
let algasMesh
new GLTFLoader().load('Models/Items/balon_azul.glb', function(gltf){
    const model = gltf.scene;
    model.traverse(object=>{
        if(object.isMesh) {
            object.castShadow=true;
...
//BALÓN
let salmonMesh
new GLTFLoader().load('Models/Items/balon.glb', function(gltf){
    const model = gltf.scene;
    model.traverse(object=>{
        if(object.isMesh) {
            object.castShadow=true;
...
//Skybox
new THREE.TextureLoader().load("skybox_2.jpg",(texture)=>{
...
function chooseMap(indexMapa){
    const nivel={
    1:'Mapas/mapa_uno.glb',
    2: 'Mapas/mapa prueba_dos.glb',
    3: 'Mapas/mapa_tres.glb'
}
...
const doublepointSound= new THREE.Audio(listener); //mi audio nuevo
...
audioLoader.load("Audio/Musica_fondo.mp3", function(buffer){
...
audioLoader.load("Audio/pickUp.mp3", function(buffer){
    pickupSound.setBuffer(buffer);
    pickupSound.setLoop(false);
    pickupSound.setVolume(VolVFX);
})
audioLoader.load("Audio/Thunder_Sound_effect.mp3", function(buffer){
    thunderSound.setBuffer(buffer);
    thunderSound.setLoop(false);
    thunderSound.setVolume(VolVFX);
})
audioLoader.load("Audio/Puntos_dobles.mp3", function(buffer){
    doublepointSound.setBuffer(buffer);
...
textureLoader.load('imagenes/Punto_azul.png', function(imagen){
        ...
        if(currentRoom != "") {
            ...
            onValue(starCountRef, (snapshot) => {
                ...
                Object.entries(data).forEach(([key, value]) => {
                    ...
                    if(!player) {           
                        new GLTFLoader().load('Characters_Matt_SingleWeapon.gltf', function(gltf){
                            const model = gltf.scene;
                            model.traverse(object=>{
                                if(object.isMesh) {
                                    if(object.name=="WoodenBat_Saw" || object.name=="Knife")object.visible=false;
                                    object.castShadow=true;
                                } 
                            });
...
//MODELO DE CLIENTE
var arrCustModels=[];
new GLTFLoader().load('Characters_Lis_SingleWeapon.gltf', function(gltf){
    const model = gltf.scene;
    model.traverse(object=>{
        if(object.isMesh) {
...
new GLTFLoader().load('Characters_Sam_SingleWeapon.gltf', function(gltf){
    const model = gltf.scene;
    model.traverse(object=>{
        if(object.isMesh) {
            if(object.name=="Knife")object.visible=false;
...
new GLTFLoader().load('Characters_Shaun_SingleWeapon.gltf', function(gltf){
    const model = gltf.scene;
    model.traverse(object=>{
        if(object.isMesh) {
            if(object.name=="Knife")object.visible=false;
        ...
        if(e.key == 'e' || e.key == 'E') {
            if(!customer.orderDelivered) {
                if (!statsPlayer.inventory.dishes[0]) {
                    showAlert('item-picked', "NO HAY BOLSA EN TU INVENTARIO!")
                    printOrders()
                }
                    ...
                    if(statsPlayer.inventory.dishes.length > 0) {
                        ...
                        showAlert('item-picked', "OBJETOS ENTREGADOS!")
                        transformDish()
                        printInventory()
                        happySound.play()
                    }
                    ...
                    showAlert('item-picked', "TE EQUIVOCASTE DE OBJETOS!")
                    printOrders()
                }
            }
            else {
...
function checkCollisions(modelBB) {
    ...
    if(modelBB.intersectsBox(basuraBB)){
        showAlert('press-button', "PULSA Q PARA TIRAR OBJETOS")
        dropItem('dish', dishes[0])
    }
    //COLISION PLATITOS
    if(modelBB.intersectsBox(platoBB)){
        showAlert('press-button', "PULSA E PARA RECOGER BOLSA")
        ...
        .forEach(customer => {       
            if(modelBB.intersectsBox(customer.boundingBox)) {
                if(!customer.orderTaken && !customer.orderDelivered && customer.spawned == true) {
                    showAlert('press-button', "PULSA E PARA TOMAR PENDIENTE")
                    takeCostumerOrder(customer)
                }
                else if(customer.orderTaken && !customer.orderDelivered && customer.spawned == true) {
                    showAlert('press-button', "PULSA E PARA ENTREGAR PENDIENTE")
    ...
    document.addEventListener('keyup', function keyPressed(e) {
        if(e.key == 'e' || e.key == 'E') {
            ...
            if(itemType == 'dish' && statsPlayer.inventory.dishes.length <= 0) {
                ...
                showAlert('item-picked', "BOLSA RECOGIDA!")
                transformDish()
                printInventory()
                dishSound.play();
            }
            ...
            if(itemType == 'ingredient') {
                if(statsPlayer.inventory.dishes.length > 0) {
                    ...
                    showAlert('item-picked', "DEBES TENER UA BOLSA PARA RECOGER UN ITEM")
                }
            }
        ...
        if(e.key == 'q' || e.key == 'Q') {
            if(itemType == 'dish') {
                if(statsPlayer.inventory.dishes.length > 0) {
                    statsPlayer.inventory.dishes[0].ingredients = []
                    statsPlayer.inventory.dishes.splice(0, 1)
                    showAlert('item-picked', "OBJETOS TIRADOS A LA BASURA")
                    transformDish()
                    printInventory()
                    trashSound.play();
                    console.log(statsPlayer)
